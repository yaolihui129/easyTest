...
开始添加接口测试功能模块
记录一些信息：
1，接口的返回值，通常包括字符串、json、xml
2，接口传入的参数，通过参数化模板加载
3，接口测试用例，通过测试用例模板实现
4，接口测试的配置信息，通过测试方案模板实现
5，在测试用例中，封装返回值对比的方法（返回值符合为指定值，或在一定返回内），封装初始化请求
6，ui测试与接口测试结合起来
7，优化代码
...
综合：
[ERROR-0001-0]:序列化外部传入的JSON字符串引发的异常

UI测试：
[ERROR-1001]:读取测试方案配置文件引发的异常.代码位置(SRC.main.Main.loadTestPlanXML)
[ERROR-1002]:序列化外部传入的JSON字符串引发的异常.代码位置(SRC.main.Main.setTestType)
...
[ERROR-1014]:解析json字符串时引发的异常.代码位置(SRC.main.Main.setTestType)
[ERROR-1015]:解析json字符串时引发的异常.代码位置(SRC.uiTest.Main.updateSettings)
[ERROR-1016]:未记录情况引发的异常.代码位置(SRC.main.Main.start)
[ERROR-1017]:未记录情况引发的异常.代码位置(SRC.uiTest.Main.run)
[ERROR-1018]:警告窗引发的异常.代码位置(SRC.common.decorator.findElement_dec)
[ERROR-1019]:未记录情况引发的异常.代码位置(SRC.interfaceTest.Main.run)
[ERROR-1020]:解析json字符串时引发的异常(SRC.interfaceTest.Main.updateSettings)
