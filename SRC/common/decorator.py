...
from SRC.common.exceptions import ScreenShotException, SwitchToWindowException, ParamsException
		...
		def wrapper(driver, *args, **kwargs):
			...
			try:
				...
				if not isNoneOrEmpty(errorMessage):
					data = getJsonData(RunResult.ERROR,
									   func.__name__,
									   '',
									   errorMessage,
									   '',
...
# 代码异常捕获
def codeException_interface_dec(level):
	def decorator(func):
		@functools.wraps(func)
		def wrapper(self, *args, **kwargs):
			ret = None
			...
			try:
				...
				errorMessage = getErrorMessage(e, '[codeException_interface_dec.%s]:' % (func.__name__))
			finally:
				if errorMessage:
					data = getJsonData(RunResult.ERROR,
									   func.__name__,
