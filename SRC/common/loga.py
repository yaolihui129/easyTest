...
from SRC.common.const import RunResult, TestType
...
def putLogForFile(data, logger):
	...
	if settings.REPORT['isWriteLog']:
		if settings.TESTTYPE==TestType.INTERFACE:
			writeLogForInterface(data, logger)  # 将日志写入文件
		elif findElementSuccess(data) and settings.REPORT['showFindElementLog']:  # 如果成功发现元素，则不打印日志
...
def manageLogRecord(sceneId=None, result='PASS', operator='add'):
	try:
		...
		putSystemLog('[ERROR-0005-0]:测试用例运行结果统计模块引发的异常.%s' % e, None, False, '', '', True)
